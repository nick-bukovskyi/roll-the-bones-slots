...
return function(test, H, loadAddon)
    ...
    local function visibleOrdinaryArtwork(frame)
        ...
        local function visit(widget)
            if
                (widget.kind == "Texture" or widget.kind == "FontString")
                and isOrdinaryVisible(widget)
            then
                count = count + 1
            end
    ...
    test("visibility migration preserves old choices and validates only owned fields", function()
        ...
        for _, version in ipairs({ false, 1, 2 }) do
            for _, visibility in ipairs({ false, "always", "active", "combat", "invalid", H.secret }) do
                ...
                local expected = type(visibility) == "string"
                        and visibility ~= "invalid"
                        and visibility
                    or "always"
    ...
    test(
        "slider and typed scale preserve a displaced center across repeated resizes and reload",
        function()
            for _, parentScale in ipairs({ 0.64, 1, 1.2 }) do
                ...
                openSettings()
                local frame, slider =
                    ns.Machine.GetFrame(), H.findTemplate("MinimalSliderWithSteppersTemplate")
    ...
    test(
        "percentage input follows numeric limits clamping and cancel semantics without helper text",
        function()
            local saved = { scale = 1.234, x = 0, y = -180 }
            local ns = login(saved)
            openSettings()
            local input, slider =
                H.findTemplate("InputBoxTemplate"),
                H.findTemplate("MinimalSliderWithSteppersTemplate")
            ...
            for _, widget in ipairs(H.widgets) do
                assert(
                    widget.template ~= "GameFontDisableSmall",
                    "scale helper text must not be constructed"
                )
            end
        end
    )
    ...
    test(
        "dragged placement stays centered when resized and edge clamping remains reachable",
        function()
    ...
    test(
        "visibility choices persist while preview ignores the live rule and buttons fill separate rows",
        function()
            local saved = {}
            local ns = login(saved)
            ...
            for _, widget in ipairs(H.widgets) do
                ...
                assert(
                    text ~= "Changes saved automatically"
                        and text ~= "Drag the highlighted display to move it"
                )
                assert(text ~= "Preview")
            end
    ...
    test(
        "combat visibility initializes from current state and survives repeated restriction transitions",
        function()
    ...
    test(
        "combat-only visibility follows player combat independently of lockdown and cast order",
        function()
    ...
    test(
        "active-only cabinet never constructs idle text beneath either buff-bar setting",
        function()
            for _, showBar in ipairs({ false, true }) do
                ...
                for _, widget in ipairs(H.widgets) do
                    ...
                    then
                        assert(
                            not idle,
                            "a second idle label would bleed through native buff artwork"
                        )
                        idle = widget
                        local ancestor = widget
                        while ancestor do
    ...
    test(
        "buff-only mode leaves all live artwork under native result ownership with no ordinary residue",
        function()
    ...
    test(
        "all visibility modes recover from eligibility and overlay changes without new frames or hooks",
        function()
            for _, mode in ipairs({ "always", "active", "combat" }) do
                local ns = login({ visibility = mode })
                openSettings()
                H.exitEditMode()
                local frame, frames, hooks, slots =
                    ns.Machine.GetFrame(), #H.frames, H.hookCount, H.auraSlotCount
