...
return function(test, H, loadAddon)
            ...
            for reel, owner in ipairs(owners) do
                ...
                for _, artwork in ipairs({ slot.button, preview(owner) }) do
                    ...
                    for index = (reel - 1) * 4 + 1, reel * 4 do
                        ...
                        for _, region in ipairs(H.reelRegions(button)) do
                            if region.drawLayer == "ARTWORK" then
                                assert(
                                    region.parent.frameLevel > light.frameLevel,
                    ...
                    for _, widget in ipairs(H.widgets) do
                        ...
                        then
                            assert(
                                widget.parent.frameLevel > light.frameLevel,
                                "a flash would cover a preview symbol"
                            )
            ...
            for _, owner in ipairs(targets(group)) do
                ...
                for _, animation in ipairs(group.animations) do
                    if animation.target == owner then
                        ...
                        elseif
                            animation.toAlpha == animation.fromAlpha and animation.toAlpha > 0
                        then
                ...
                if rank == 4 then
                    assert(
                        rises[1].toAlpha < rises[2].toAlpha and rises[2].toAlpha < rises[3].toAlpha
                    )
                    assert(
                        holds[3].duration > holds[1].duration
                            and holds[3].duration > holds[2].duration
                    )
                end
            end
        end
    ...
    test(
        "each readable Roll starts all native timelines once without result or refresh detection",
        function()
            local ns = login()
            quiet()
