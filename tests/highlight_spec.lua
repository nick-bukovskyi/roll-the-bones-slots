...
return function(test, H, loadAddon)
    ...
    test(
        "local eye hides only the highlight and retains settings dragging samples and tooltip ownership",
        function()
            local ns = login()
    ...
    test(
        "hidden highlights survive native selection suppression and reject restricted local changes",
        function()
            local ns = login()
            local eye, selection = openSettings()
            click(eye)
    ...
    test(
        "EQOL absent unsupported late and recreated eyes attach once and latest explicit action wins",
        function()
            local ns = login()
            local eye, selection = openSettings()
    ...
    test(
        "obsolete providers stop controlling the highlight after supported provider replacement",
        function()
            login()
