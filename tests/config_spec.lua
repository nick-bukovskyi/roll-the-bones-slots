return function(test, ns, harness)
    ...
    test(
        "scale edits clamp to the supported bounds and reject invalid input without mutation",
        function()
            local root = Config.Initialize(nil)
    ...
    test(
        "position updates preserve fractions and reject a partial invalid move atomically",
        function()
            local root = Config.Initialize(nil)
    ...
    test(
        "a newer schema stays untouched and cannot be changed by older settings controls",
        function()
            local unknown = { retained = true }
            local root = {
                schemaVersion = 3,
