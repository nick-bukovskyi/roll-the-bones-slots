return function(test, ns, harness)
    ...
    test("initialization preserves player choices and unrelated saved data", function()
        ...
        local root = { schemaVersion = 1, scale = 1.25, x = -73.5, y = 40.75,
            animationEnabled = false, extra = unknown }
    ...
    test("corrupt fields recover individually", function()
        local root = { schemaVersion = "invalid", scale = 9, x = 90.25, y = math.huge,
            animationEnabled = "false", extra = "preserved" }
        Config.Initialize(root)
        eq(root.schemaVersion, 1)
        eq(root.scale, 1)
        eq(root.x, 90.25)
    ...
    test("invalid roots and secret fields never become persisted preferences", function()
        ...
        local root = Config.Initialize({ schemaVersion = harness.secret, scale = harness.secret,
            x = harness.secret, y = 12, animationEnabled = harness.secret })
        eq(root.schemaVersion, 1)
        eq(root.scale, 1)
        eq(root.x, 0)
        eq(root.y, 12)
    ...
    test("scale edits accept the supported bounds and reject invalid input without mutation", function()
        local root = Config.Initialize(nil)
        eq(Config.SetScale(0.6), true)
        eq(Config.GetScale(), 0.6)
        eq(Config.SetScale(1.8), true)
        eq(Config.GetScale(), 1.8)
        for _, invalid in ipairs({ 0.59, 1.81, 0 / 0, math.huge, -math.huge,
            "1", false, {}, harness.secret }) do
    ...
    test("position updates preserve fractions and reject a partial invalid move atomically", function()
        ...
        for _, invalid in ipairs({ 0 / 0, math.huge, -math.huge, "3", false, {}, harness.secret }) do
    ...
    test("reset changes only owned fields and the same saved root", function()
        local unknown = { retained = true }
        local root = Config.Initialize({ scale = 1.5, x = 35, y = -87,
            animationEnabled = false, extra = unknown })
    ...
    test("a newer schema stays untouched and cannot be changed by older settings controls", function()
        local unknown = { retained = true }
        local root = { schemaVersion = 2, scale = 1.7, x = "future position", y = false,
            animationEnabled = "future mode", extra = unknown }
        eq(Config.Initialize(root), root)
        eq(_G.RollTheBonesSlotsDB, root)
