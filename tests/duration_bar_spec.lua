...
return function(test, H, loadAddon)
    ...
    test(
        "duration preference adds and repairs only its field while preserving false and future schemas",
        function()
            local ns = loadAddon()
    ...
    test(
        "native bar binds remaining duration for the same buffs below the unchanged footer labels",
        function()
            login()
            ...
            for _, region in pairs(footer.button.bindings) do
                assert(
                    region.parent.frameLevel > bar.frameLevel,
                    "footer labels must stay above the native fill"
                )
            end
            ...
            assert(
                iconPoint[3] > barPoint[3] and iconPoint[3] + icon.width < barPoint[3] + bar.width
            )
            assert(
                -iconPoint[4] > -barPoint[4]
                    and -iconPoint[4] + icon.height < -barPoint[4] + bar.height
            )
            eq(icon.width, 16)
            eq(icon.height, 16)
            eq(#H.timers, 0)
    ...
    test(
        "duration toggle changes only bar visibility and persists independently of reduced motion",
        function()
    ...
    test(
        "duration control rejects combat edits and recovers across repeated previews overlays and resets",
        function()
            local ns = login()
            local checkbox = selectCabinet()
