...
return function(test, H, loadAddon)
  ...
  local function provider()
    local library = {}
    local state = { dialog = CreateFrame("Frame", nil, UIParent), selected = false, closeCalls = 0 }
  ...
  test("EQOL and chest settings hand off in both directions without stale edits or duplicate work", function()
    ...
    local input, dropdown = H.findTemplate("InputBoxTemplate"), H.findTemplate("WowStyle1DropdownTemplate")
    for _ = 1, 8 do
      input.focused = true
      input:SetText("170")
  ...
  test("optional public dialog close is discovered when it appears and after provider replacement", function()
  ...
  test("restricted editor suspension cancels input and defers EQOL handoff until editing is available", function()
    local ns, selection = prepare()
    local _, state, selectProvider = provider()
    local panel = selectChest(selection)
