...
return function(test, H, loadAddon)
    ...
    test(
        "every result preview replaces empty text with either duration setting and every live visibility choice",
        function()
            for _, mode in ipairs({ "always", "active", "combat" }) do
                for _, animate in ipairs({ false, true }) do
                    ...
                    for _ = 1, 3 do
                        for _ = 1, 5 do
                            local expected = ns.Game.Results[index] and ns.Game.Results[index].label
                                or "empty"
                            ...
                            index = index % 5 + 1
                            expected = ns.Game.Results[index] and ns.Game.Results[index].label
                                or "empty"
    ...
    test(
        "result and empty reel previews never share a visible symbol layer during cycles spins and resizing",
        function()
            local ns = login()
            selectCabinet()
    ...
    test(
        "combat live preview and hidden transitions restore one complete presentation without stale samples",
        function()
            for _, mode in ipairs({ "always", "active", "combat" }) do
                for _, showBar in ipairs({ false, true }) do
                    for _, initialCombat in ipairs({ false, true }) do
                        local ns = login(
                            { visibility = mode, durationBarEnabled = showBar },
                            initialCombat
                        )
    ...
    test(
        "reported combat sequence stays idempotent with nil or false inactive Edit Mode and no diagnostics",
        function()
            local sequence = {
                "SPELLS_CHANGED",
            ...
            for _, inactive in ipairs({ "nil", "false" }) do
                ...
                for cycle = 1, 10 do
                    for index, event in ipairs(sequence) do
                        ...
                        H.combat, H.playerCombat =
                            event == "PLAYER_REGEN_DISABLED", event == "PLAYER_REGEN_DISABLED"
                        H.fire(event)
    ...
    test(
        "loading while Edit Mode is already active selects one sample and exits to the saved visibility rule",
        function()
            for _, mode in ipairs({ "always", "active", "combat" }) do
    ...
    test(
        "overlapping overlays keep all content hidden until the final cause clears in live and preview states",
        function()
            local exitOrders = {
            ...
            for _, mode in ipairs({ "always", "active", "combat" }) do
                ...
                for _, preview in ipairs({ true, false }) do
                    ...
                    for _, exitOrder in ipairs(exitOrders) do
                        ...
                        footerState(
                            ns,
                            preview and ns.Game.Results[2].label
                                or mode == "active" and "hidden"
                                or "empty",
                            true
                        )
                        reelState(
                            ns,
                            preview and "result" or mode == "active" and "hidden" or "empty"
                        )
    ...
    test(
        "unavailable or unreadable eligibility clears live artwork and recovers without retaining samples",
        function()
            for _, mode in ipairs({ "always", "active", "combat" }) do
                ...
                for _, fault in ipairs({
                    ...
                    H.fire(
                        ...
                        "player",
                        fault[1] .. tostring(#H.pointWrites),
                        1214909
                    )
                    H[fault[1]] = fault[2]
