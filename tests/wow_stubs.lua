...
function H.install()
    ...
    local function check(value)
        ...
        while current do
            assert(
                not rawget(current, "sealed"),
                "add-on accessed a restricted aura object after initialization"
            )
    ...
    function methods:GetCenter()
        ...
        if p then
            return UIParent.width / (2 * self.scale) + p[3],
                UIParent.height / (2 * self.scale) + p[4]
        end
        return 0, 0
    end
    ...
    function methods:CreateAnimation(kind)
        ...
        local animation = setmetatable(
            { kind = kind, parent = self, order = 1, duration = 0, startDelay = 0, endDelay = 0 },
            {
                __index = function(_, key)
    ...
    function methods:SetDurationBar(bar, options, ...)
        ...
        self.durationBarOptions =
            { direction = options.direction, interpolation = options.interpolation }
    end
    function methods:AddAuraSlot(key, filter, options)
        ...
        assert(
            type(filters) == "table" and next(filters),
            "native slot needs explicit spell filters"
        )
        ...
        assert(
            rawget(button, "animationGroups") == nil,
            "native aura buttons must not own add-on animations"
        )
