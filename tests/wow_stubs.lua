...
function H.install()
    ...
    function methods:SetSize(w, h) check(self); assert(type(w) == "number" and type(h) == "number"); self.width, self.height = w, h end
    function methods:GetWidth() check(self); return self.width end
    function methods:GetHeight() check(self); return self.height end
    function methods:GetSize() check(self); return self.width, self.height end
    function methods:SetPoint(point, relative, relativePoint, x, y)
        check(self); assert(type(point) == "string" and type(relative) == "table" and type(relativePoint) == "string")
        assert(type(x) == "number" and type(y) == "number")
        if not self.points[point] then table.insert(self.pointOrder, point) end
        self.points[point] = { relative, relativePoint, x, y }
        H.pointWrites[#H.pointWrites + 1] = self
    end
    function methods:GetPoint(index)
        check(self); assert(type(index) == "number")
        local point = self.pointOrder[index]
        if point then return point, unpackValues(self.points[point]) end
    end
    function methods:SetAllPoints(relative)
        check(self); assert(type(relative) == "table"); self.allPoints = relative
    ...
    function methods:ClearAllPoints() check(self); self.points, self.pointOrder = {}, {} end
    function methods:SetScale(value) check(self); assert(type(value) == "number"); self.scale = value end
    function methods:GetScale() check(self); return self.scale end
    function methods:GetEffectiveScale() check(self); return self.scale * (self.parent and self.parent:GetEffectiveScale() or 1) end
    function methods:GetCenter()
        check(self)
        if self == UIParent then return self.width / 2, self.height / 2 end
        if rawget(self, "allPoints") then return self.allPoints:GetCenter() end
        local p = self.points.CENTER
        if p then return UIParent.width / (2 * self.scale) + p[3], UIParent.height / (2 * self.scale) + p[4] end
        return 0, 0
    end
    function methods:GetRect() check(self); local x, y = self:GetCenter(); return x - self.width / 2, y - self.height / 2, self.width, self.height end
    function methods:CanBeAccessedInContext() check(self); return self.accessible end
    function methods:SetShown(value)
        check(self); assert(type(value) == "boolean"); local changed = self.shown ~= value; self.shown = value
        if changed then
            local function dispatch(current)
                local event = value and "OnShow" or "OnHide"
                if current.scripts[event] then current.scripts[event](current) end
                for _, child in ipairs(current.children) do if child.shown then dispatch(child) end end
            end
            dispatch(self)
        end
    end
    ...
    function methods:IsShown() check(self); return self.shown end
    function methods:IsVisible() check(self); return self.shown and (not self.parent or self.parent:IsVisible()) end
    function methods:SetScript(event, fn) check(self); assert(type(event) == "string" and (fn == nil or type(fn) == "function")); self.scripts[event] = fn end
    function methods:RegisterEvent(event) check(self); assert(type(event) == "string"); self.events[event] = true; return true end
    function methods:UnregisterEvent(event) check(self); self.events[event] = nil end
    function methods:RegisterUnitEvent(event, unit) check(self); H.eq(unit, "player"); self.events[event] = unit; return true end
    function methods:CreateTexture(name, layer)
        check(self); assert(name == nil and type(layer) == "string")
        local texture = object("Texture", self); texture.drawLayer = layer; return texture
    end
    function methods:CreateFontString(name, layer, template) check(self); assert(name == nil and layer == "OVERLAY" and type(template) == "string"); return object("FontString", self, template) end
    function methods:SetText(text) check(self); assert(type(text) == "string"); self.text = text end
    ...
    function methods:SetTexCoord(left, right, top, bottom, ...)
        check(self); H.eq(select("#", ...), 0); assert(type(left) == "number" and type(right) == "number" and type(top) == "number" and type(bottom) == "number")
        self.texCoords = { left, right, top, bottom }
        H.textureWrites = H.textureWrites + 1
    end
    function methods:SetDrawLayer(layer, sublayer)
        check(self); assert(type(layer) == "string" and (sublayer == nil or type(sublayer) == "number"))
    ...
    function methods:SetFrameStrata(value) check(self); assert(value == "MEDIUM" or value == "DIALOG" or value == "HIGH"); self.strata = value end
    function methods:SetFrameLevel(value) check(self); assert(type(value) == "number"); self.frameLevel = value end
    function methods:GetFrameLevel() check(self); return self.frameLevel end
    function methods:SetClipsChildren(value) check(self); assert(type(value) == "boolean"); self.clipsChildren = value end
    function methods:SetClampedToScreen(value) check(self); H.eq(value, true) end
    function methods:SetMovable(value) check(self); assert(type(value) == "boolean"); self.movable = value end
    ...
    function methods:SetNumeric(value) check(self); H.eq(value, false) end
    function methods:SetMaxLetters(value) check(self); H.eq(value, 7) end
    function methods:HasFocus() check(self); return self.focused end
    function methods:ClearFocus()
        check(self); local previous = self.focused; self.focused = false
        if previous and self.scripts.OnEditFocusLost then self.scripts.OnEditFocusLost(self) end
    end
    function methods:SetChecked(value) check(self); assert(type(value) == "boolean"); self.checked = value end
    function methods:GetChecked() check(self); return self.checked end
    function methods:Init(value, minimum, maximum, steps, formatters)
        check(self); H.eq(self.template, "MinimalSliderWithSteppersTemplate"); H.eq(minimum, 0.6); H.eq(maximum, 1.8)
        assert(math.abs(steps - 24) < 1e-9 and type(formatters) == "table"); self.value = value
    end
    ...
    function methods:SetSystem(system) check(self); H.eq(self.template, "EditModeSystemSelectionTemplate"); H.eq(system:GetSystemName(), "Roll the Bones Slots") end
    function methods:ShowHighlighted() check(self); self.selectionStyle = "highlighted"; self:Show() end
    function methods:ShowSelected() check(self); self.selectionStyle = "selected"; self:Show() end
    function methods:SetUnit(unit) check(self); H.eq(self.kind, "AuraContainer"); H.eq(unit, "player"); self.unit = unit end
    ...
    local templates = { DialogBorderTranslucentTemplate = "Frame", UIPanelCloseButton = "Button",
        MinimalSliderWithSteppersTemplate = "Frame", InputBoxTemplate = "EditBox", UICheckButtonTemplate = "CheckButton",
        UIPanelButtonTemplate = "Button", EditModeSystemSelectionTemplate = "Frame", CustomAuraContainerTemplate = "AuraContainer" }
    ...
    _G.IsLoggedIn = function() return H.loggedIn end
    _G.InCombatLockdown = function() return H.combat end
    _G.InCinematic = function() return H.cinematic end
    _G.C_PetBattles = { IsInBattle = function() return H.petBattle end }
    _G.C_Secrets = { ShouldAurasBeSecret = function() return H.restricted end }
    _G.C_SpecializationInfo = { GetSpecialization = function() return 2 end,
        GetSpecializationInfo = function(index) H.eq(index, 2); return H.spec end }
    _G.C_SpellBook = { IsSpellKnown = function(id) H.eq(id, 1214909); return H.known end }
    _G.C_AddOns = { GetAddOnMetadata = function(addon, key) H.eq(addon, "RollTheBonesSlots"); H.eq(key, "Version"); return "dev" end }
    _G.C_UnitAuras = setmetatable({}, { __index = function() error("add-on must never read live aura data") end })
    _G.C_Timer = { After = function(delay, fn) H.eq(delay, 0); assert(type(fn) == "function"); table.insert(H.timers, fn) end }
    _G.WOW_PROJECT_ID, _G.WOW_PROJECT_MAINLINE = 1, 1
    _G.SlashCmdList, _G.SLASH_ROLLTHEBONESSLOTS1 = {}, nil
    _G.print = function(message) table.insert(H.messages, message) end
    _G.Settings = setmetatable({}, { __index = function() error("obsolete Blizzard Settings category used") end })
    _G.MinimalSliderWithSteppersMixin = { Event = { OnValueChanged = "OnValueChanged" } }
    _G.EditBox_ClearHighlight = function(box) H.eq(box.kind, "EditBox") end
    _G.EditModeSystemSelectionMixin, _G.EditModeSystemMixin = {}, {}
    _G.EditModeMagnetismManager = nil
    _G.EventRegistry = { RegisterCallback = function(_, event, fn, owner)
        assert(event == "EditMode.Enter" or event == "EditMode.Exit"); assert(type(fn) == "function" and type(owner) == "table")
        assert(not H.callbacks[event], "duplicate Edit Mode callback"); H.callbacks[event] = { fn = fn, owner = owner }
    end }
    _G.hooksecurefunc = function(target, name, callback)
        ...
        H.hookCount = H.hookCount + 1; local original = target[name]
        target[name] = function(...) original(...); callback(...) end
    end
    function H.newManager()
        local manager = object("Frame", UIParent); manager.active, manager.snap = false, false
        function manager:IsEditModeActive() return self.active end
        function manager:IsSnapEnabled() return self.snap end
        function manager:SelectSystem(system) assert(type(system) == "table") end
        ...
        _G.EditModeManagerFrame = manager; return manager
    end
    H.newManager()
end
...
function H.enterEditMode()
    EditModeManagerFrame.active = true; EditModeManagerFrame:Show()
    local callback = assert(H.callbacks["EditMode.Enter"]); callback.fn(callback.owner)
end

function H.exitEditMode()
    EditModeManagerFrame.active = false
    local callback = assert(H.callbacks["EditMode.Exit"]); callback.fn(callback.owner)
end
...
function H.flushTimers()
    local pending = H.timers; H.timers = {}
    for _, fn in ipairs(pending) do fn() end
end

function H.findTemplate(template)
    for _, frame in ipairs(H.frames) do if frame.template == template then return frame end end
end

function H.button(text)
    for _, frame in ipairs(H.frames) do if frame.kind == "Button" and rawget(frame, "text") == text then return frame end end
    error("button missing: " .. text)
end

function H.cabinet()
    for _, frame in ipairs(H.frames) do if frame.width == 400 and frame.height == 246 then return frame end end
