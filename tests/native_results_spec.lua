...
return function(test, H, loadAddon)
        ...
        for _, region in ipairs(H.reelRegions(parent)) do
            if region.kind == "Texture" then
                ...
                elseif region.points.CENTER and region.points.CENTER[3] == laneX then
                    ...
                    assert(not rows[row], "duplicate strip row")
                    rows[row] = region
                end
            end
        end
    ...
    test(
        "clipped reel windows stay above both cabinet backgrounds through visibility and preview transitions",
        function()
                    ...
                    local function checkLayers()
                        for _, well in ipairs(wells) do
                            for _, cabinet in ipairs(cabinets) do
                                -- Check the clipping boundary, not just its high-level symbol descendants
                                assert(
                                    well.frameLevel > cabinet.frameLevel,
                                    "cabinet background can cover the entire clipped reel window"
                                )
                            end
                        end
                        for _, slot in ipairs(H.nativeSlots) do
                            if slot.key:match("^result") then
                                local carrier = slot.container.parent
                                eq(carrier.parent.clipsChildren, true)
                                assert(slot.button.frameLevel > carrier.parent.frameLevel)
                            end
                        end
                    end
                    checkLayers()
    ...
    test("native reel and shared buff construction never reads back a result", function()
        local ns = loadAddon()
        ...
        for displayIndex = 1, 4 do
            ...
            function methods:AddAuraSlot(key, filter, options)
                ...
                function buttonMethods:SetPoint(point, relative, relativePoint, x, y)
                    eq(point, "TOPLEFT")
                    eq(relative, container)
                    eq(relativePoint, "TOPLEFT")
                    eq(x, 0)
                    eq(y, 0)
                    prepared.anchor = true
                end
                function buttonMethods:SetCancelAuraButtons(value)
                    eq(value, nil)
                    prepared.cancel = true
                end
                function buttonMethods:SetHideTooltipInCombat(value)
                    eq(value, true)
                    prepared.tooltip = true
                end
                function buttonMethods:EnableMouse(value)
                    eq(footer, false)
                    eq(value, false)
                    prepared.mouseDisabled = true
                end
                function buttonMethods:SetSize(width, height)
                    assert(prepared.anchor and prepared.cancel and prepared.tooltip)
                    if not footer then
                        eq(prepared.mouseDisabled, true)
                    end
                    eq(width, 91)
                    eq(height, 153)
                end
                ...
                return setmetatable({}, {
                    __index = function()
                        error("slot return value inspected")
                    end,
                })
            end
            function methods:SetEnabled(enabled)
                eq(self, container)
                eq(enabled, true)
                eq(initialized, footer and 1 or 4)
                calls.enabled = true
            end
            _G.CreateFrame = function(kind, name, owner, template)
                eq(kind, "AuraContainer")
                eq(name, nil)
                eq(owner, parent)
                eq(template, "CustomAuraContainerTemplate")
                return container
            end
            local result
            if footer then
                result = ns.Game.CreateBuffDisplay(parent, function(...)
                    eq(select("#", ...), 1)
                    local button = ...
                    button:SetSize(91, 153)
                    initialized = initialized + 1
                end)
            else
                result = ns.Game.CreateReelDisplay(parent, displayIndex, function(...)
                    eq(select("#", ...), 3)
                    local button, definition, reelIndex = ...
                    local resultIndex = initialized + 1
                    eq(reelIndex, displayIndex)
                    eq(definition, ns.Game.Results[resultIndex])
                    eq(definition.spellID, expected[resultIndex][1])
                    eq(definition.label, expected[resultIndex][2])
            ...
            eq(result, container)
            assert(calls.anchored and calls.unit and calls.enabled)
        end
        _G.CreateFrame = originalCreateFrame
        eq(totalSlots, 13)
        eq(#ns.Game.Results, 4)
    end)
            ...
            for _, showBar in ipairs({ true, false }) do
                ...
                for _, slot in ipairs(H.nativeSlots) do
                    ...
                    if button.bindings.SetSpellName then
                        ...
                        local insets = assert(
                            rawget(button, "hitRectInsets"),
                            "footer hover covers the cabinet"
                        )
    ...
    test(
        "native result strips cover dim idle artwork and the native footer covers the idle message",
        function()
            local ns = loadAddon()
            ...
            local function opaqueAbove(texture, empty)
                assert(
                    texture.parent.frameLevel > empty.parent.frameLevel,
                    "native background must cover empty artwork on a strictly higher frame"
                )
            ...
            for index = 1, 13 do
                ...
                if index <= 12 then
                    ...
                    for _, child in ipairs(H.widgets) do
                        if
                            rawget(child, "text") == "Try yer luck, matey!"
                            and child.parent.parent == ns.Machine.GetFrame()
                        then
                            idleLabel = child
                        end
                    end
    ...
    test(
        "continuous native and idle strips share a full lead-in and retain complete neighboring textures",
        function()
            local ns = loadAddon()
            ns.Config.Initialize(nil)
            ns.Machine.Initialize()
            ...
            for index = 1, 12 do
                ...
                for lane = 1, reelIndex == 1 and 1 or 3 do
                    ...
                    for row = -1, ns.Art.SpinRows + 1 do
                        ...
                        if row > -1 then
                            eq(
                                texture.points.CENTER[4] - rows[row - 1].points.CENTER[4],
                                -ns.Art.Pitch
                            )
                        end
                        if row >= 2 then
    ...
    test("prebuilt strip backing covers every viewport throughout motion and settle", function()
        ...
        for index = 1, 12 do
            ...
            for lane = 1, index <= 4 and 1 or 3 do
                local _, backgrounds =
                    strip(slot.button, ns.Art.Pitch, (lane - 1) * ns.Art.LanePitch)
        ...
        for _ = 0, 180 do
            for _, entry in ipairs(geometry) do
                ...
                covers(
                    entry.lanes[lane],
                    offset,
                    offset + entry.carrier.parent.height,
                    entry.carrier.width,
                    x
                )
            end
