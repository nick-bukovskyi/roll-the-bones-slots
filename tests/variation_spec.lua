...
return function(test, H, loadAddon)
    ...
    test(
        "all six cosmetic pairs preserve exact native rank dice counts without artwork writes",
        function()
            WithDraws({ 1, 1, 1, 2, 2, 1, 2, 2, 3, 1, 3, 2 }, function(drawCount)
                ...
                eq(drawCount(), 0)
                local alpha, writes, widgets, points =
                    #H.alphaWrites, H.textureWrites, #H.widgets, #H.pointWrites
                ...
                for attempt, pair in ipairs(cases) do
                    ...
                    for rank = 1, 4 do
                        ...
                        for reel = 1, 3 do
                            ...
                            local symbol =
                                CenterSymbol(slot.button, (variant - 1) * ns.Art.LanePitch)
    ...
    test(
        "reduced motion still chooses variants while duplicate or unreadable casts do not",
        function()
    ...
    test(
        "repeat spins, stop, hidden UI and restriction recovery preserve the latest live pair",
        function()
