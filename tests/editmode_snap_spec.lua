...
return function(test, H, loadAddon)
    local function WithWorld(run)
        local ns = loadAddon()
        local names = { "UIParent", "CreateFrame", "EditModeManagerFrame", "EditModeMagnetismManager", "EditModeSystemMixin" }
        local saved = {}
        ...
        local function Rect(left, bottom, width, height, scale)
            ...
            function object:GetRect() return self.left, self.bottom, self.width, self.height end
            function object:GetCenter() return self.left + self.width / 2, self.bottom + self.height / 2 end
            function object:GetScale() return self.scale end
            function object:GetEffectiveScale() return self.scale end
            ...
            function object:SetPoint(point, relative, relativePoint, x, y)
                H.eq(point, "CENTER"); H.eq(relative, UIParent); H.eq(relativePoint, "CENTER")
        ...
        ns.Game.CanConfigure = function() return world.allowed end
        EditModeManagerFrame = {
            IsEditModeActive = function() return true end,
            IsSnapEnabled = function() return true end,
        }
        EditModeSystemMixin = {
            ...
            GetSnapOffsets = function(_, info) return info.testOffsetX or 0, info.testOffsetY or 0 end,
        }
        EditModeMagnetismManager = {
            topLevelParentCenterX = 960, topLevelParentCenterY = 540,
            topLevelParentLeft = 0, topLevelParentRight = 1920,
            topLevelParentBottom = 0, topLevelParentTop = 1080,
            topLevelParentWidth = 1920, topLevelParentHeight = 1080,
        ...
        CreateFrame = function(kind, name, parent)
            H.eq(kind, "Frame"); H.eq(name, nil); H.eq(parent, UIParent)
            local preview = { scripts = {} }
            function preview:SetAllPoints(relative) H.eq(relative, UIParent) end
            function preview:SetFrameStrata(strata) H.eq(strata, "HIGH") end
            function preview:EnableMouse(value) H.eq(value, false) end
            function preview:SetScript(event, fn) self.scripts[event] = fn end
            function preview:Show() self.shown = true end
            function preview:Hide()
                self.shown = false
                if self.scripts.OnHide then self.scripts.OnHide(self) end
            end
            function preview:CreateLine(name, layer, template)
                H.eq(name, nil); H.eq(layer, "OVERLAY"); H.eq(template, "MagnetismPreviewLineTemplate")
                return { Hide = function() end, ClearAllPoints = function() end,
                    Setup = function(_, info, anchor) assert(info.frame); H.eq(anchor, "CenterVertical") end }
            end
    ...
    test("grid placement converts screen displacement to saved cabinet scale on both axes", function()
        WithWorld(function(Snap, world)
            for _, scale in ipairs({ 0.6, 1, 1.8 }) do
                ...
                world.infos = {
                    { frame = UIParent, point = "CENTER", relativePoint = "CENTER", isHorizontal = true,
                        testOffsetX = 240 / scale },
                    { frame = UIParent, point = "CENTER", relativePoint = "CENTER", isHorizontal = false,
    ...
    test("element corner snaps resolve to UIParent instead of anchoring to the target", function()
        WithWorld(function(Snap, world)
            ...
            target.SetPoint = function() error("must not mutate snap target") end
            world.infos = { { frame = target, point = "TOPLEFT", relativePoint = "TOPRIGHT", isCornerSnap = true } }
            Snap.Begin(world.frame, world.selection)
            Snap.Finish(true)
    ...
    test("restrictions arriving during drag cancel snapping without another native geometry query", function()
        WithWorld(function(Snap, world)
            world.infos = { { frame = UIParent, point = "CENTER", relativePoint = "CENTER", isHorizontal = true } }
            Snap.Begin(world.frame, world.selection)
