...
return function(test, H, loadAddon)
    local function WithWorld(run)
        ...
        local function Rect(left, bottom, width, height, scale)
            local object =
                { left = left, bottom = bottom, width = width, height = height, scale = scale }
    ...
    test(
        "grid placement converts screen displacement to saved cabinet scale on both axes",
        function()
    ...
    test(
        "restrictions arriving during drag cancel snapping without another native geometry query",
        function()
            WithWorld(function(Snap, world)
                world.infos = {
                    {
