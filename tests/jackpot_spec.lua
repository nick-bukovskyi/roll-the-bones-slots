...
return function(test, H, loadAddon)
    ...
    local function VisibleSample(widget)
        ...
        if
            ...
            and widget.points.CENTER[2] == "CENTER"
        then
    ...
    test(
        "exclusive chest appears only at native Jackpot centers, never ordinary strip rows",
        function()
            local ns = loadAddon()
    ...
    test(
        "preview chest is exclusive to labeled Jackpot centers and footer across every sample",
        function()
            local ns = loadAddon()
            ns.Config.Initialize(nil)
            ns.Machine.Initialize()
            ...
            for _ = 1, 5 do
                ...
                for _, texture in ipairs(H.widgets) do
                    if texture.kind == "Texture" and VisibleSample(texture) then
                        ...
                        if id then
                            local point = assert(
                                texture.points.CENTER,
                                "authored symbols retain their visual center"
                            )
