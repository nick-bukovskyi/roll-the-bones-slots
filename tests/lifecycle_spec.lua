return function(test, H, loadAddon)
    ...
    test("TOC bootstrap waits for saved data and creates widgets and hooks once", function()
        ...
        eq(H.categoryCount, 0)
    end)

    test(
        "unsupported build or flavor preserves saved data and creates no native display",
        function()
    ...
    test(
        "native reels and footer use only the four exact results and no Lua aura event reads",
        function()
            login()
            nativeEnabled(true)
            for _, container in ipairs(H.containers()) do
    ...
    test(
        "readable successful casts spin once and all three reels stop without idle updates",
        function()
            login()
    ...
    test(
        "continuous ordinary carriers move the same prebuilt artwork without inner clipping or texture updates",
        function()
            local ns = login()
            local moving = carriers()
            local alphaCount = #H.alphaWrites
            local textureWrites, pointCount, widgetCount =
                H.textureWrites, #H.pointWrites, #H.widgets
            assertSettled()
            spin("landing")
            ...
            local first, second, third =
                moving[1].points.TOPLEFT[4],
                moving[2].points.TOPLEFT[4],
                moving[3].points.TOPLEFT[4]
            ...
            for index = pointCount + 1, #H.pointWrites do
                ...
                assert(
                    widget == moving[1] or widget == moving[2] or widget == moving[3],
                    "animation moved something other than an ordinary carrier"
                )
            end
            for _, carrier in ipairs(moving) do
                eq(carrier:IsShown(), true)
    ...
    test(
        "repeat rolls and interruptions reset every carrier without touching native descendants",
        function()
            local ns = login()
            spin("first")
            H.advance(0.95)
    ...
    test(
        "editor settings validate scale, preserve canceled text and stop reduced-motion spins",
        function()
            local ns = login()
            selectCabinet()
            local slider = H.findTemplate("MinimalSliderWithSteppersTemplate")
    ...
    test(
        "ordinary editor dragging round-trips scaled position and rejects unreachable saved placement",
        function()
    ...
    test(
        "repeated Edit Mode sessions reuse controls and cancel uncommitted input on deselection",
        function()
            local ns = login()
            selectCabinet()
            local input = H.findTemplate("InputBoxTemplate")
    ...
    test(
        "death ends cosmetic motion and restricted display-size changes apply after restrictions lift",
        function()
    ...
    test(
        "spec, missing spell, overlays and loading clear work and recover without rebuilding",
        function()
            login()
            local cabinet = H.cabinet()
