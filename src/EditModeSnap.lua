...
local GEOMETRY_METHODS = {
    ...
    "GetSelectionOffset", "GetCombinedSelectionOffset", "GetCombinedCenterOffset", "GetSnapOffsets",
}
local PARENT_COORDINATES = {
    "topLevelParentCenterX", "topLevelParentCenterY", "topLevelParentLeft", "topLevelParentRight",
    "topLevelParentBottom", "topLevelParentTop", "topLevelParentWidth", "topLevelParentHeight",
}
local ANCHORS = {
...
local function IsReadableObject(object)
    if issecretvalue(object) or type(object) ~= "table" or not canaccesstable(object) then return false end
...
local function HasReadableRect(frame)
    if not IsReadableObject(frame) then return false end
    local left, bottom, width, height = frame:GetRect()
    local x, y = frame:GetCenter()
    return IsNumber(left) and IsNumber(bottom) and IsNumber(width) and IsNumber(height)
        and width > 0 and height > 0 and IsNumber(x) and IsNumber(y)
end

local function HasReadableGeometry(frame, selection)
    if not HasReadableRect(frame) or not HasReadableRect(selection) then return false end
    local scale = frame:GetScale()
    if not IsNumber(scale) or scale <= 0 then return false end
    for index = 1, 2 do
        local _, _, _, x, y = selection:GetPoint(index)
        if not IsNumber(x) or not IsNumber(y) then return false end
    end
    return true
end
...
function mover:GetFrameMagneticEligibility(candidate)
    if not IsReadableObject(candidate) or candidate == self.frame then return false, false end
    if type(candidate.GetScaledSelectionSides) ~= "function"
        ...
        or not HasReadableGeometry(candidate, candidate.Selection) then return false, false end
...
local function GetSnapInfos()
    if not active or not ns.Game.CanConfigure() then return nil end
    local manager, magnetism = EditModeManagerFrame, EditModeMagnetismManager
    if not manager or not magnetism or not manager:IsEditModeActive() or not manager:IsSnapEnabled()
        or not HasReadableGeometry(mover.frame, mover.Selection) or not HasReadableRect(UIParent) then
        return nil
    end
    local parentScale = UIParent:GetEffectiveScale()
    if not IsNumber(parentScale) or parentScale <= 0 then return nil end
    for _, key in ipairs(PARENT_COORDINATES) do
        if not IsNumber(magnetism[key]) then return nil end
    end
...
local function ApplyPlacement(infos)
    ...
    frame:SetPoint("CENTER", UIParent, "CENTER", (centerX - parentX) / scale, (centerY - parentY) / scale)
end

function Snap.Begin(frame, selection)
    Snap.Finish(false)
    if not ns.Game.CanConfigure() then return end
    mover.frame, mover.Selection = frame, selection
    for _, method in ipairs(GEOMETRY_METHODS) do mover[method] = EditModeSystemMixin[method] end
...
function Snap.Finish(applySnap)
    if applySnap then
        local infos = GetSnapInfos()
        if infos then ApplyPlacement(infos) end
    end
    active = false
    HideLines()
